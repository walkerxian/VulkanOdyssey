...
VkResult SocSwapChain::submitCommandBuffers(const VkCommandBuffer *buffers, uint32_t *imageIndex) {
...
void SocSwapChain::createRenderPass() {
  ...
  //定义一个图形管线的子通道,并指定了颜色附件引用和深度附件引用
  ...
  //定义了一个SubpassDependency，确保在颜色输出和早期片段测试阶段之前，外部的操作（如图形获取）已经完成

  //确保渲染之前，所有必要的工作已经完成
  VkSubpassDependency dependency = {};
  dependency.dstSubpass = 0;
  //访问掩码： 指定了在绘制颜色和深度颜色之前必须完成的操作类型
  //
...
void SocSwapChain::createDepthResources() {
...
VkSurfaceFormatKHR SocSwapChain::chooseSwapSurfaceFormat(
...
VkPresentModeKHR SocSwapChain::chooseSwapPresentMode(
...
VkExtent2D SocSwapChain::chooseSwapExtent(const VkSurfaceCapabilitiesKHR &capabilities) {
...
VkFormat SocSwapChain::findDepthFormat() {
